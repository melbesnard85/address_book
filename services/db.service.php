...
namespace
{
	class db
	{
		...
		public function __clone() {
			throw new \melwin\database\Exception('Cannot clone static DB');
		}
		public static function get($settings = null) {
			return new \melwin\database\DBC($settings);
		}
		public static function getc($settings = null, \melwin\cache\ICache $c = null) {
			if($c === null) { $c = \melwin\cache\cache::inst(); }
			return new \melwin\database\DBCCached($settings, $c);
		}
	}
}

namespace melwin\database
{
	...
	class DBC implements IDB
	{
		...
		public function __construct($drv = null) {
			...
			if(is_string($drv)) {
				$drv = new \melwin\database\Settings($drv);
			}
			if($drv instanceof Settings) {
				$tmp = '\\melwin\\database\\' . $drv->type . '_driver';
				if(!class_exists($tmp)) {
	...
	class DBCCached extends DBC
	{
		...
		public function __construct($settings = null, \melwin\cache\ICache $c = null) {
